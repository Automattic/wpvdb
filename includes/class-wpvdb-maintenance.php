...
class Maintenance {
	...
	private static function cleanup_old_logs() {
		...
		$logs = get_option( 'wpvdb_logs', array() );
		if ( ! is_array( $logs ) ) {
			return;
		}
	...
	private static function cleanup_orphaned_embeddings() {
		...
		if ( $deleted_count > 0 ) {
			...
			Logger::info( 'Cleaned up orphaned embeddings', array( 'count' => $deleted_count ) );
		}
	}
	...
	private static function deep_optimize_database() {
		...
		if ( $table_status && isset( $table_status['Data_free'] ) ) {
			...
			if ( $data_length > 0 && ( $data_free / $data_length ) > 0.25 ) {
				...
				Logger::info(
					'Rebuilt fragmented database table',
					array(
	...
	private static function cleanup_old_queue_items() {
		$queue = get_option( 'wpvdb_embedding_queue', array() );
	...
	public static function get_maintenance_status() {
		...
		if ( function_exists( 'as_get_scheduled_actions' ) ) {
			$tasks = array( 'wpvdb_daily_maintenance', 'wpvdb_weekly_maintenance', 'wpvdb_monthly_maintenance' );
