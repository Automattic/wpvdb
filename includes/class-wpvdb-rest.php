...
class REST {
	...
	public static function register_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( __CLASS__, 'handle_embed' ),
				'permission_callback' => array( __CLASS__, 'default_permission_check' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( __CLASS__, 'handle_vectors' ),
				'permission_callback' => array( __CLASS__, 'default_permission_check' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( __CLASS__, 'handle_reembed' ),
				'permission_callback' => array( __CLASS__, 'default_permission_check' ),
			)
		);
	}
	...
	public static function default_permission_check() {
		...
		Logger::debug( 'Permission check', array( 'require_auth' => $require_auth ) );
		...
		Logger::debug( 'Access granted', array( 'user_id' => get_current_user_id() ) );
		return true;
	}
	...
	public static function handle_embed( WP_REST_Request $request ) {
		...
		if ( empty( $api_key ) ) {
			return new WP_Error( 'configuration_error', 'API key not configured. Please contact site administrator.', array( 'status' => 400 ) );
		}
		...
		if ( ! is_array( $chunks ) || empty( $chunks ) ) {
			return new WP_Error( 'chunking_error', 'Failed to chunk text.', array( 'status' => 500 ) );
		}
	...
	public static function handle_query( \WP_REST_Request $request ) {
		...
		if ( $has_provided_vector && ! current_user_can( self::get_required_capability() ) ) {
			return new \WP_Error( 'rest_forbidden_vector_query', __( 'You do not have permission to query with a provided vector.', 'wpvdb' ), array( 'status' => 403 ) );
		}

		if ( ! $has_provided_vector && empty( $data['query'] ) ) {
			return new \WP_Error( 'missing_query', __( 'Query text is required', 'wpvdb' ), array( 'status' => 400 ) );
		}
	...
	public static function insert_embedding_row( $doc_id, $chunk_id, $chunk_content, $summary, $embedding, $model = '', $doc_type = 'post', $chunk_index = null ) {
		...
		if ( $has_vector ) {
			try {
				...
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					Logger::error( 'Exception in insert_embedding_row: ' . $e->getMessage() );
				}
				$result = false;
			}
		} else {
	...
	private static function validate_provided_query_vector( $vector ) {
		if ( ! is_array( $vector ) ) {
			return new \WP_Error( 'invalid_vector', __( 'Provided vector must be an array.', 'wpvdb' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $vector as $value ) {
			if ( ! is_numeric( $value ) ) {
				return new \WP_Error( 'invalid_vector', __( 'Provided vector contains non-numeric values.', 'wpvdb' ), array( 'status' => 400 ) );
			}

			$float = (float) $value;
			if ( ! is_finite( $float ) ) {
				return new \WP_Error( 'invalid_vector', __( 'Provided vector contains non-finite values.', 'wpvdb' ), array( 'status' => 400 ) );
			}
		...
		if ( $sum_sq <= 1e-30 ) {
			return new \WP_Error( 'invalid_vector', __( 'Provided vector must have non-zero magnitude.', 'wpvdb' ), array( 'status' => 400 ) );
		}
	...
	public static function cosine_distance( $vec1, $vec2 ) {
		...
		if ( ! is_array( $vec1 ) || ! is_array( $vec2 ) ) {
			Core::log_error(
				'cosine_distance received non-array input',
				array(
					'v1' => $vec1,
					'v2' => $vec2,
				)
