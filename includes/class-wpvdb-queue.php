...
class WPVDB_Queue {
	...
	public static function process_item( $item ) {
		...
		if ( ! $post_id ) {
			Core::log_error( 'Invalid post ID in queue task', array( 'item' => $item ) );
			return false;
		}
		...
		if ( ! $post || ! is_object( $post ) ) {
			Core::log_error( 'Post not found', array( 'post_id' => $post_id ) );
			return false;
		}
	...
	private static function process_post( $post, $model, $provider = '' ) {
		...
		if ( empty( $chunks ) ) {
			Core::log_error( 'No chunks generated for post', array( 'post_id' => $post->ID ) );
			return false;
		}
